...
static int ts_to_string(char *str, int len, int ts)
{
    int h, m, s;
    h = ts/360000;  ts -= 360000*h;
    m = ts/  6000;  ts -=   6000*m;
    s = ts/   100;  ts -=    100*s;
    return snprintf(str, len, "%d:%02d:%02d.%02d", h, m, s, ts);
}
                    ...
                    int ts_start, int ts_end, int raw)
{
    ...
    if (!rects)
        return AVERROR(ENOMEM);
    sub->rects = rects;
    sub->end_display_time = FFMAX((int)sub->end_display_time, 10 * duration);
